...
const WatchingHistoryItem:React.FC<WatchingHistoryItemProps> = ({el}) => {
    return (
        <Link href={`/anime/${el.animeId}`} className="flex gap-4 px-3 py-3 bg-white/20 border-2 border-[#43aa52] rounded-3xl w-[420px]">
            ...
            <div className="w-full flex flex-col gap-2 py-2">
                <p
                    className={`max-w-[250px] text-white text-2xl overflow-x-hidden  break-keep truncate ${styles.title}`}>
                    {el.title}
                </p>
                <p>
                    {
