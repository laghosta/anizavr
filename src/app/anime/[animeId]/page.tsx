...
const getAnime = async (id: number) => {
    const res = await customFetch(`api/getAnime/${id}`, "GET").then((res) =>
        res.json()
    );
    return res;
};
const getSimilarAnime = async (id: number) => {
    const res = await customFetch(`api/getSimilarAnime/${id}`, "GET").then(
        (res) => res.json()
    );
    return res;
};
const getRelatedAnime = async (id: number) => {
    const res = await customFetch(`api/getRelatedAnime/${id}`, "GET").then(
        (res) => res.json()
    );
    return res;
};
            ...
            <div className="w-full h-full">
                <div className="flex flex-col mt-5 xl:mt-10 lg:mt-8 md:mt-6 text-center justify-between max-w-[960px] mx-auto w-screen">
                    ...
                    <div className="flex flex-col xl:flex-row lg:flex-row md:flex-row sm:flex-col items-center xl:h-[400px] lg:h-[400px] md:h-[400px] h-full w-full mt-5 xl:mt-10 lg:mt-8 md:mt-6 gap-6">
                        ...
                        <ul className="flex flex-col justify-between  items-baseline max-w-[300px] max-h-[400px] xl:h-[400px] lg:h-[400px] md:h-[400px] gap-2">
                            <li>
                                Статус:{" "}
                                <span
                            ...
                            <li className="flex items-center gap-2 flex-wrap max-w-[300px]">
                    ...
                    {data && (
                        <Player
                            link={`https:${
                                data?.kodikDetails?.results![0].link
                            }`}
                            animeId={props.params.animeId}
